...
const initDatabase = async () => {
  try {
    ...
    try {
      const lastLoginColumnCheck = await queryWithRetry(`
        SELECT column_name
        FROM information_schema.columns
      ...
      if (
        ...
        console.warn('Warning: Could not add last_login column:', error.message);
      }
    }
    ...
    console.log('âœ… Password_reset_tokens table ready');

    // NEW TABLE: login_logs
    // Track all login attempts with timestamp and IP address
    // ─────────────────────────────────────────────────────────────
    await queryWithRetry(`
    ...
    // Track account deletion requests from users
    // ─────────────────────────────────────────────────────────────
    await queryWithRetry(`
    ...
    // Currently used for role_permissions.
    // ─────────────────────────────────────────────────────────────
    await queryWithRetry(`
    ...
    // Seed default role permissions if not already set
    ...
    await queryWithRetry(
      ...
      ['role_permissions', JSON.stringify(defaultRolePermissions)]
    );
